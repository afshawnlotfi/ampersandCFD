        ...
        for aKey in keys:
            ...
            if (objName == "k" or objName == "epsilon" or objName == "omega"):
                s_file += write_turbulence_boundary_condition(
                    patch=aKey, purpose=anItem['purpose'], property=anItem['property'])
            elif (objName == "p"):
...
{"""
    ...
    for patch in meshSettings['geometry']:
        if (patch['type'] == 'triSurfaceMesh'):
            U_file += write_vector_boundary_condition(
                patch=patch['name'], purpose=patch['purpose'], property=patch['property'])
    U_file += """
}"""
    return U_file
    ...
    if (meshSettings['internalFlow'] == False):
        ...
        for aKey in keys:
            ...
            nut_file += write_turbulence_boundary_condition(
                patch=aKey, purpose=anItem['purpose'], property=anItem['property'], wallFunction="nutkWallFunction")

    # If internal flow, set the boundary conditions for STL patches
    for patch in meshSettings['geometry']:
        if (patch['type'] == 'triSurfaceMesh'):
            nut_file += write_turbulence_boundary_condition(
                patch=patch['name'], purpose=patch['purpose'], property=patch['property'], wallFunction="nutkWallFunction")
    nut_file += """
}"""
    return nut_file

...
addLayers       {meshSettings.snappyHexSteps.addLayers};"""
    ...
    geometry = f"""\ngeometry\n{{"""
    for patch_name, geometry_patch in meshSettings.geometry.items():
        ...
        if (geometry_patch.type == 'triSurfaceMesh'):
            added_geo = f"""\n
    {patch_name}
    {{
        type {geometry_patch.type};
        name {patch_name[:-4]};
        regions
        {{
            {patch_name[:-4]}
            ...
            if (geometry_patch.featureEdges):
                features += f"""

        {{
            file \"{patch_name[:-4]}.eMesh\";
            level {geometry_patch.featureLevel};
        }}"""
            if (geometry_patch.purpose == 'inlet' or geometry_patch.purpose == 'outlet'):
                patchType = 'patch'
        ...
        {{
            ...
            {{
                {patch_name[:-4]}
                {{
                    level ({geometry_patch.refineMin} {geometry_patch.refineMax});
                    patchInfo
                    {{
                        type {patchType};
                    }}
            ...
            elif (geometry_patch.purpose == 'cellZone'):
                patchType = 'cellZone'
                if geometry_patch.property[1] == True:  # patches will be added
                    refinementSurfaces += f"""
        {patch_name[:-4]}
        {{
            level (0 0);
            cellZone {patch_name[:-4]};
            faceZone {patch_name[:-4]};
        ...
        {{
            ...
            elif (geometry_patch.purpose == 'refinementRegion' or geometry_patch.purpose == 'refinementSurface'):
                pass

            elif (geometry_patch.purpose == 'baffle'):
                patchType = 'wall'
                refinementSurfaces += f"""
        {patch_name[:-4]}
        {{
        ...
        elif (geometry_patch.type == 'searchableBox'):
            added_geo = f"""
    {patch_name}
    {{
        type {geometry_patch.type};
        min ({geometry_patch.min[0]} {geometry_patch.min[1]} {geometry_patch.min[2]});
        max ({geometry_patch.max[0]} {geometry_patch.max[1]} {geometry_patch.max[2]});
    }}"""
        geometry += added_geo
    geometry += f"""
    ...
    refinementRegions = f""
    for patch_name, geometry_patch in meshSettings.geometry.items():
        if (geometry_patch.type == 'searchableBox'):
            refinementRegions += f"""
        {patch_name}
        {{
            mode inside;
            levels ((1E15 {geometry_patch.refineMax}));
        }}"""
        elif (geometry_patch.type == 'triSurfaceMesh'):
            if (geometry_patch.purpose == 'refinementSurface'):
                refinementRegions += f"""
        {patch_name[:-4]}
        {{
            mode distance;
            levels ((1E-4 {geometry_patch.property}));
        }}"""
            elif (geometry_patch.purpose == 'refinementRegion'):
                refinementRegions += f"""
        {patch_name[:-4]}
        {{
            mode inside;
            levels ((1E15 {geometry_patch.property}));
        }}"""
            elif (geometry_patch.purpose == 'cellZone'):
                refinementRegions += f"""
        {patch_name[:-4]}
        {{
            mode inside;
            levels ((1E15 {geometry_patch.property[0]}));
        }}"""
...
{{
    ...
    for patch_name, geometry_patch in meshSettings.geometry.items():
        if (geometry_patch.type == 'triSurfaceMesh'):
            if (geometry_patch.purpose == 'wall'):  # If the surface is a wall, add layers
                layerControls += f"""
            "{patch_name[:-4]}.*"
            {{
                nSurfaceLayers {geometry_patch.nLayers};
            }}"""
            elif (geometry_patch.purpose == 'baffle'):  # If the surface is a baffle, add layers
                layerControls += f"""
            "{patch_name[:-4]}.*"
            {{
                nSurfaceLayers {1};

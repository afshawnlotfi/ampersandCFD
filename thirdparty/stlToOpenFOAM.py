...
def compute_curvature(mesh, curvature_type='mean'):
    ...
    """
    curvature_filter = vtk.vtkCurvatures()  # type: ignore
...
def compute_surface_normals(mesh):
    ...
    """
    normals_filter = vtk.vtkPolyDataNormals()  # type: ignore
...
def is_point_inside(mesh, point):
    ...
    """
    enclosed_points = vtk.vtkSelectEnclosedPoints()  # type: ignore
    enclosed_points.SetSurfaceData(mesh)

    points = vtk.vtkPoints()  # type: ignore
    points.InsertNextPoint(point)

    polydata = vtk.vtkPolyData()  # type: ignore
...
def find_inside_point(mesh, center_of_mass, min_bounds, max_bounds, initial_distance=0.1, step_factor=0.5) -> tuple[float, float, float]:
    """
    ...
    """
    direction = np.array(
        # Fixed direction for deterministic approach
        [1.0, 1.0, 1.0], dtype=np.float64)
    direction /= np.linalg.norm(direction)
    distance = initial_distance
    inside_point = np.array(
        center_of_mass, dtype=np.float64) - direction * distance
...
def find_outside_point(mesh, center_of_mass, min_bounds, max_bounds, initial_distance=0.1, buffer=10, max_buffer=20):
    ...
    # Max distance based on the bounding box and max buffer
    max_distance = np.linalg.norm(bbox_size) + max_buffer
    ...
    extended_min_bounds, extended_max_bounds = create_extended_bounding_box(
        min_bounds, max_bounds, buffer)
    for i in range(3):
    ...
    # Debug information
    print(f"Outside point distance: {distance}, max_distance: {
          max_distance}, outside_point: {outside_point}")
...
def write_json_report(filename, volume=None, surface_area=None, center_of_mass=None,
    ...
    if min_bounds is not None and max_bounds is not None:
        center, lengths = compute_geometry_center_and_lengths(
    ...
    if min_edge_length is not None and max_edge_length is not None:
        report["min_edge_length"] = {
            "value": min_edge_length, "units": "meters"}
        report["max_edge_length"] = {
            "value": max_edge_length, "units": "meters"}
    ...
    if inside_point is not None:
        report["inside_point"] = {
            "coordinates": inside_point, "units": "meters"}
    if outside_point is not None:
        report["outside_point"] = {
            "coordinates": outside_point, "units": "meters"}
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="STL Geometry Analysis using VTK")
    ...
    parser.add_argument("--surface_area", action="store_true",
                        help="Compute surface area")
    parser.add_argument("--center_of_mass", action="store_true",
                        help="Compute center of mass")
    parser.add_argument("--bounding_box", action="store_true",
                        help="Compute bounding box")
    parser.add_argument("--curvature", action="store_true",
                        help="Compute curvature")
    parser.add_argument("--surface_normals",
                        action="store_true", help="Compute surface normals")
    parser.add_argument("--facet_areas", action="store_true",
                        help="Compute facet areas")
    parser.add_argument("--edge_lengths", action="store_true",
                        help="Compute edge lengths")
    parser.add_argument("--aspect_ratios", action="store_true",
                        help="Compute aspect ratios")
    parser.add_argument("--inside_outside_points", action="store_true",
                        help="Compute inside and outside points")
    parser.add_argument("--generate_blockMeshDict", action="store_true",
                        help="Generate blockMeshDict for OpenFOAM")
    parser.add_argument("--buffer", type=float, default=10,
                        help="Buffer size to add around the bounding box")
    parser.add_argument("--max_buffer", type=float, default=20,
                        help="Maximum buffer size allowed for the bounding box")
...
def main():
    ...
    if args.volume or args.surface_area:
        mass_properties = vtk.vtkMassProperties()  # type: ignore
    ...
    if args.inside_outside_points and center_of_mass is not None and min_bounds is not None and max_bounds is not None:
        inside_point = find_inside_point(
            mesh, center_of_mass, min_bounds, max_bounds)
        outside_point = find_outside_point(mesh, center_of_mass, min_bounds, max_bounds,
                                           buffer=args.buffer, max_buffer=args.max_buffer)  # Ensure within buffer range

    # Write JSON report
    write_json_report(args.filename, volume, surface_area, center_of_mass, min_bounds, max_bounds, curvature_values, surface_normals,
                      min_area, max_area, min_edge_length, max_edge_length, min_aspect_ratio, max_aspect_ratio, inside_point, outside_point)

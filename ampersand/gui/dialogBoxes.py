...
class sphereDialog(QDialog):
    ...
    def load_ui(self):
        ui_file = QFile("src/gui/templates/createSphereDialog.ui")
        ...
        self.window.lineEditSphereRadius.setValidator(
            QDoubleValidator(0.0, 1e10, 3))

    def prepare_events(self):
        self.window.pushButtonOK.clicked.connect(self.on_pushButtonOK_clicked)
        self.window.pushButtonCancel.clicked.connect(
...
class inputDialog(QDialog):
    def __init__(self, prompt="Enter Input", input_type: Type[T] = str):
        super().__init__()
    ...
    def load_ui(self):
        ui_file = QFile("src/gui/templates/inputDialog.ui")
...
class vectorInputDialog(QDialog):
    ...
    def load_ui(self):
        ui_file = QFile("src/gui/templates/vectorInputDialog.ui")
        ui_file.open(QFile.ReadOnly)
        self.window = loader.load(ui_file, None)
        ui_file.close()
...
def yesNoCancelDialogDriver(prompt="Save changes to current case files", title="Save Changes"):
    msg = QMessageBox().question(None, title, prompt, QMessageBox.Yes |
...
def inputDialogDriver(prompt="Enter Input", input_type: Type[T] = str) -> T:
    dialog = inputDialog(prompt=prompt, input_type=input_type)
    dialog.window.exec()
    dialog.window.show()
    input_value = dialog.input
    if input_value is None:
        raise ValueError("Input Dialog Box Closed")
    return input_type(input_value)


def vectorInputDialogDriver(prompt="Enter Input", element_type: Type[T] = str):
    dialog = vectorInputDialog(prompt=prompt, input_type=element_type)
    dialog.window.exec()
    dialog.window.show()

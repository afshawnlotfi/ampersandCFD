...
class mainWindow(QMainWindow):
    ...
    def prepare_vtk(self):
        ...
        self.ren = vtk.vtkRenderer() # type: ignore
    ...
    def showSTL(self,stlFile=r"C:\Users\mrtha\Desktop\GitHub\foamAutoGUI\src\pipe.stl"):
        # Read stl
        try:
            self.reader = vtk.vtkSTLReader() # type: ignore
    ...
    def render3D(self):  # self.ren and self.iren must be used. other variables are local variables
        # Create a mapper
        mapper = vtk.vtkPolyDataMapper() # type: ignore
        mapper.SetInputConnection(self.reader.GetOutputPort())
        # Create an actor
        actor = vtk.vtkActor() # type: ignore
        actor.SetMapper(mapper)
        actor.GetProperty().EdgeVisibilityOn()
        colors = vtk.vtkNamedColors() # type: ignore
        self.ren.SetBackground(colors.GetColor3d("SlateGray"))
        self.ren.AddActor(actor)
        style = vtk.vtkInteractorStyleTrackballCamera() # type: ignore
        self.iren.SetInteractorStyle(style)
        camera = vtk.vtkCamera() # type: ignore

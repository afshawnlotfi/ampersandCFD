...
import numpy as np
import math
from ampersand.models.settings import BCPatch, BoundingBox, MeshSettings, SearchableBoxGeometry
from thirdparty.stlToOpenFOAM import find_inside_point, is_point_inside, read_stl_file
...
class STLAnalysis:
    ...
    @staticmethod
    def calc_nx_ny_nz(domain_size: BoundingBox, target_cell_size: float):
        (minX, maxX, minY, maxY, minZ, maxZ) = domain_size.to_tuple()
    ...
    @staticmethod
    def compute_bounding_box(stl_file_path: str):
    ...
    def calc_mesh_settings(stlBoundingBox: BoundingBox, nu=1e-6, rho=1000., U=1.0, maxCellSize=0.5, sizeFactor=1.0,
        ...
        AmpersandIO.printMessage(f"Domain size: x({domain_size.minx:6.3f}~{domain_size.maxx:6.3f}) y({domain_size.miny:6.3f}~{
                                 domain_size.maxy:6.3f}) z({domain_size.minz:6.3f}~{domain_size.maxz:6.3f})", GUIMode=GUI, window=window)
